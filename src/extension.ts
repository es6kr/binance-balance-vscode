...
export function activate(context: vscode.ExtensionContext) {
    ...
    const showBalancesCommand = vscode.commands.registerCommand('binanceBalance.showBalances', () => {
    ...
    vscode.window.showInformationMessage('Binance Balance Monitor activated! Check Output panel for logs.');
...
async function showDetailedBalances() {
    try {
        if (!binanceApi.isConfigured()) {
            vscode.window.showWarningMessage('Please configure your Binance API credentials first.');
            return;
        }
        ...
        vscode.window.withProgress({
            ...
            items.push({
                ...
                detail: `Spot: ${estimatedBalance.spotUSDT.toFixed(2)} USDT | Margin: ${estimatedBalance.marginUSDT.toFixed(2)} USDT`
            });
            ...
            if (items.length === 1) { // Only summary item
